...
void usbi_remove_pollfd(struct libusb_context *ctx, int fd) {
	...
	usbi_mutex_lock(&ctx->pollfds_lock);
	list_for_each_entry(ipollfd, &ctx->pollfds, list, struct usbi_pollfd) if (ipollfd->pollfd.fd == fd) {
			found = 1;
			break;
		}
...
const struct libusb_pollfd **LIBUSB_CALL libusb_get_pollfds(
	...
	list_for_each_entry(ipollfd, &ctx->pollfds, list, struct usbi_pollfd) ret[i++] = (struct libusb_pollfd *) ipollfd;
	ret[cnt] = NULL;
...
void usbi_handle_disconnect(struct libusb_device_handle *handle) {
	...
	while (1) {
		...
		to_cancel = NULL;
		list_for_each_entry(cur, &HANDLE_CTX(handle)->flying_transfers, list, struct usbi_transfer) if (
