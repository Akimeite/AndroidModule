...
static int cache_config_descriptors(struct libusb_device *dev, HANDLE hub_handle, char *device_id) {
	...
	for (i = 0, r = LIBUSB_SUCCESS;; i++) {
		...
		cd_buf_short.req.ConnectionIndex = (ULONG)
		priv->port;
		...
		// Actual call
		cd_buf_actual->ConnectionIndex = (ULONG)
		priv->port;
		...
		cd_data = (PUSB_CONFIGURATION_DESCRIPTOR) ((UCHAR * )
		cd_buf_actual + sizeof(USB_DESCRIPTOR_REQUEST));
...
static int init_device(struct libusb_device *dev, struct libusb_device *parent_dev,
	...
	if (priv->depth != 0) {    // Not a HCD hub
		...
		conn_info.ConnectionIndex = (ULONG)
		port_number;
...
static int
windows_get_config_descriptor(struct libusb_device *dev, uint8_t config_index, unsigned char *buffer, size_t len, int *host_endian) {
	struct windows_device_priv *priv = _device_priv(dev);
...
const char *hub_driver_names[] = {"USBHUB", "USBHUB3", "USB3HUB", "NUSB3HUB", "RUSB3HUB", "FLXHCIH", "TIHUB3", "ETRONHUB3", "VIAHUB3",
                                  "ASMTHUB3", "IUSB3HUB", "VUSB3HUB", "AMDHUB30"};
			...
			switch (err) {
				...
				default:
					...
					for (i = 0;; i++) {
						...
						dev_interface_details = get_interface_details_filter(ctx, &dev_info, &dev_info_data,
...
static int winusbx_submit_control_transfer(int sub_api, struct usbi_transfer *itransfer) {
	...
	if (((setup->request_type & (0x03 << 5)) == LIBUSB_REQUEST_TYPE_STANDARD)
		...
		if (!WinUSBX[sub_api].ControlTransfer(wfd.handle, *setup, transfer->buffer + LIBUSB_CONTROL_SETUP_SIZE, size, NULL,
		                                      wfd.overlapped)) {
...
static int _hid_get_report_descriptor(struct hid_device_priv *dev, void *data, size_t *size) {
	...
	if (dev->input_report_size) {
		...
		/* report count */
		d[i++] = 0x95;
		d[i++] = (uint8_t) dev->input_report_size - 1;
	...
	if (dev->output_report_size) {
		...
		/* report count */
		d[i++] = 0x95;
		d[i++] = (uint8_t) dev->output_report_size - 1;
	...
	if (dev->feature_report_size) {
		...
		/* report count */
		d[i++] = 0x95;
		d[i++] = (uint8_t) dev->feature_report_size - 1;
			...
			switch (setup->request) {
				case LIBUSB_REQUEST_GET_DESCRIPTOR:
					r = _hid_get_descriptor(priv->hid, wfd.handle, LIBUSB_REQ_RECIPIENT(setup->request_type),
					                        (setup->value >> 8) & 0xFF, setup->value & 0xFF, transfer->buffer + LIBUSB_CONTROL_SETUP_SIZE,
					                        &size);
					break;
