...
struct libusb_device *usbi_get_device_by_session_id(struct libusb_context *ctx,
	...
	usbi_mutex_lock(&ctx->usb_devs_lock);
	{
		list_for_each_entry(dev, &ctx->usb_devs, list, struct libusb_device) if (dev->session_data == session_id) {
...
int API_EXPORTED libusb_clear_halt(libusb_device_handle *dev,
                                   unsigned char endpoint) {
	LOGW("**k602_debug**===1=fun=%s====line=%d==file=%s=",__FUNCTION__,__LINE__,__FILE__);
	return 0;
	/*
...
int API_EXPORTED libusb_free_streams(libusb_device_handle *dev,
	...
	{
		RETURN(usbi_backend->free_streams(dev, endpoints, num_endpoints),
		int);
	} else {
		RETURN(LIBUSB_ERROR_NOT_SUPPORTED,
		int);
...
static void usbi_log_str(struct libusb_context *ctx,
	...
	switch (level) {
		case LIBUSB_LOG_LEVEL_NONE:
			break;    // XXX add to avoid warning when compiling with clang
