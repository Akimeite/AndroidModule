...
public class CameraHelper implements Camera.PreviewCallback {
	...
	private Object previewDisplayView;
	...
	public void init() {
		...
		if (isMirror && previewDisplayView instanceof View) {
			((View) previewDisplayView).setScaleX(-1);
		}
	}
...
//				previewSize = parameters.getPreviewSize();
				...
				for (Camera.Size size : supportedPreviewSizes) {
					Log.i("supportedPreviewSize", size.width + " * " + size.height);
				}
//				if (supportedPreviewSizes != null && supportedPreviewSizes.size() > 0) {
//					previewSize = getBestSupportedSize(supportedPreviewSizes, previewViewSize);
//				}
				parameters.setPreviewSize(previewViewSize.x, previewViewSize.y);
				...
				if (previewDisplayView instanceof SurfaceTexture) {
					mCamera.setPreviewTexture(((SurfaceTexture) previewDisplayView));
				} else if (previewDisplayView instanceof TextureView) {
...
//		previewSize = null;
	...
	private Camera.Size getBestSupportedSize(List<Camera.Size> sizes, Point previewViewSize) {
		if (null == sizes || sizes.isEmpty() || null == previewViewSize) {
	...
	public static final class Builder {
		...
		/**
		 * 预览显示的view，目前仅支持SurfaceTexture/SurfaceView/TextureView
		 */
		private Object previewDisplayView;
		...
		public Builder previewOn(Object val) {
			if (val instanceof SurfaceTexture || val instanceof TextureView || val instanceof SurfaceView) {
				...
				throw new RuntimeException("you must preview on a SurfaceTexture or a TextureView or a SurfaceView");
			}
		}
